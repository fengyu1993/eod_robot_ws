// http://docs.ros.org/en/melodic/api/moveit_tutorials/html/doc/visualizing_collisions/visualizing_collisions_tutorial.html
#include <ros/ros.h>
...
void computeCollisionContactPoints(InteractiveRobot& robot)
{
    ...
    if(c_res.collision)
    {
        ...
        if(c_res.contact_count > 0)
        {
            ...
            collision_detection::getCollisionMarkersFromContacts(markers, "base_link", c_res.contacts, 
                                                                color, ros::Duration(), 0.01);  
...
{///interactive_robot_state
    ros::init(argc, argv, "visualizing_collisions_tutorial");
    ros::NodeHandle nh;

    /*create a PlanningScene for collision checking*/
    InteractiveRobot robot("right_arm");
